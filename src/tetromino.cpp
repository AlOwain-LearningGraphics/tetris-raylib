...
void tetromino::reset()
{
    ...
    change_pos({GetRandomValue(2, 8), 20}, m_rotation);
}

bool tetromino::logic(input_type next_move)
{
    switch (next_move) {
        case DOWN: return change_pos({m_pos.m_x, m_pos.m_y - 1}, m_rotation);
        case LEFT: return change_pos({m_pos.m_x - 1, m_pos.m_y}, m_rotation);
        case RIGHT: return change_pos({m_pos.m_x + 1, m_pos.m_y}, m_rotation);
        case ROTATE: return change_pos({m_pos.m_x, m_pos.m_y}, !m_rotation);
        default: return true;
    }
}
...
tetromino::input_type tetromino::input()
{
    ...
    if (IsKeyPressed(KEY_UP)) return ROTATE;
    return NO_MOVE;
}
